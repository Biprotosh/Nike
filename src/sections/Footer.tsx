            ...
            <div className="flex justify-between items-start gap-20 flex-wrap max-lg:flex-col">
                ...
                <div className="flex flex-1 justify-between lg:gap-10 gap-20 flex-wrap">
                    {footerLinks.map(({ links, title }) => (
                        <div key={title}>
                            <h4 className="text-white font-montserrat text-2xl leadingnor font-medium mb-6">
                                {title}
                            </h4>
                            <ul>
                                {links.map((link) => (
                                    <li
                                        key={link.name}
                                        className="mt-3 text-white-400 font-montserrat text-base leading-normal hover:text-slate-gray duration-150 cursor-pointer"
                                    >
                                        <a>{link.name}</a>
                                    </li>
                                ))}
